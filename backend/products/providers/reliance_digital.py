...
class RelianceDigitalProvider(BaseProvider):
    """Reliance Digital provider — uses ProductExtractor for extraction."""

    def __init__(self):
        super().__init__()
        self.platform_name = "Reliance Digital"
    ...
    def get_product_details(self, product_url: str) -> Optional[ProductData]:
        try:
            ...
            raw = extractor.extract(product_url, "Reliance Digital")
            ...
            logger.warning("Reliance Digital extraction failed for %s: %s", product_url, e)
            return None
