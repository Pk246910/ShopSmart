    ...
    <div className="app-wrapper" style={{ maxWidth: 1100, margin: "0 auto", padding: "2rem 1.5rem 3rem" }}>
      ...
      <div className="no-print" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.75rem", marginBottom: "1.5rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem" }}>
          ...
          <ShareButton title={product.title} price={bestDeal?.price} url={`${window.location.origin}/analysis/${id}`} />
        </div>
      </div>
        ...
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          {(
            <div style={{ marginBottom: "1rem" }}>
              <img src={getProductImage(product.image_url)} alt={product.title} style={{ maxHeight: 120, maxWidth: "100%", objectFit: "contain", borderRadius: 8 }} onError={handleImageError} />
            </div>
          )}
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem" }}>
                <thead>
                  <tr style={{ color: "var(--text-muted)", borderBottom: "1px solid var(--border-color)" }}>
                    ...
                    <th style={{ padding: "0.5rem", textAlign: "left" }}>Link</th>
                  </tr>
                </thead>
                <tbody>
                  {listings.map((l, i) => (
                    <tr key={i} style={{ background: i === 0 ? "rgba(16,185,129,0.06)" : "transparent", borderBottom: "1px solid var(--border-subtle)" }}>
                      ...
                      <td style={{ padding: "0.5rem" }}>
                        {l.source_url ? (
                          <a href={l.source_url} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: "0.3rem", padding: "0.2rem 0.5rem", background: "rgba(6,182,212,0.1)", color: "var(--accent-cyan)", borderRadius: "4px", fontWeight: 600, fontSize: "0.7rem", textDecoration: "none", border: "1px solid rgba(6,182,212,0.2)" }}>
                            View on {l.platform} ↗
                          </a>
                        ) : (
