...
Full-stack e-commerce price comparison app: **Django 6 + DRF** backend + **React 18 + Vite 8** frontend. Compare offers across 8 Indian platforms with AI-powered analysis, deal scoring, and decision reports.
...
- **Multi-AI Providers** — Gemini → OpenAI → Claude → rule-based fallback chain
- **Decision Report** — Printable full-page report with radar charts, bar charts, price trends, and AI insights
...
| POST | `/api/analyze-url/` | — | Paste URL → AI comparison |
...
| GET | `/api/ai-status/` | admin | AI provider status |
...
82 tests covering: health check, products API, wishlist, URL analysis, AI scoring engine, comparison engine, auth (register/login/profile/password), admin stats, promote/demote, price alerts, forgot password, token refresh.
...
- **3 AI providers** — Gemini, OpenAI, Claude (with automatic fallback)
